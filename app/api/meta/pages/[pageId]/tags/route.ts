// app/api/meta/pages/[pageId]/tags/route.ts
export const runtime = "nodejs";

import { NextRequest, NextResponse } from "next/server";
import { ensureEnvOrThrow, makeApiUrl, readCookie } from "../../../../../_utils/backend";

const noStore = {
  "content-type": "application/json",
  "Cache-Control": "no-store, no-cache, must-revalidate, max-age=0",
  Pragma: "no-cache",
  Expires: "0",
} as const;

function resolveAuth(req: NextRequest) {
  const fromHeader = req.headers.get("authorization");
  if (fromHeader) return fromHeader;
  const bearer = readCookie(req.headers.get("cookie") || "", "access_token");
  if (bearer) return `Bearer ${bearer}`;
  return undefined;
}

/** Helper: parse backend response safely (json or text) */
async function parseSafe(res: Response) {
  try {
    return await res.clone().json();
  } catch {
    try {
      return await res.clone().text();
    } catch {
      return {};
    }
  }
}

function baseHeaders(req: NextRequest) {
  return {
    Accept: "application/json",
    "X-Requested-With": "XMLHttpRequest",
    ...(resolveAuth(req) ? { Authorization: resolveAuth(req)! } : {}),
  } as Record<string, string>;
}

/** Carry all incoming query params (locale, page, per_page, etc.) */
function buildBackendUrl(req: NextRequest, pageId: string) {
  const be = new URL(makeApiUrl(`meta/pages/${encodeURIComponent(pageId)}/tags`));
  req.nextUrl.searchParams.forEach((v, k) => be.searchParams.set(k, v));
  return be.toString();
}

/* ===================== GET /api/meta/pages/:pageId/tags ===================== */
export async function GET(
  req: NextRequest,
  ctx: { params: Promise<{ pageId: string }> }
) {
  try {
    ensureEnvOrThrow();
    const { pageId } = await ctx.params;

    const upstream = await fetch(buildBackendUrl(req, pageId), {
      method: "GET",
      headers: baseHeaders(req),
      cache: "no-store",
    });

    const data = (await parseSafe(upstream)) ?? {};
    return new NextResponse(typeof data === "string" ? data : JSON.stringify(data), {
      status: upstream.status || 200,
      headers: noStore,
    });
  } catch (e: any) {
    return NextResponse.json(
      { status: "error", message: e?.message || "Proxy GET meta tags failed" },
      { status: 502, headers: noStore }
    );
  }
}

/* ===================== POST /api/meta/pages/:pageId/tags ===================== */
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ pageId: string }> }
) {
  try {
    ensureEnvOrThrow();
    const { pageId } = await ctx.params;

    const contentType = req.headers.get("content-type") || "";
    const headers = { ...baseHeaders(req) } as Record<string, string>;
    let body: any = null;

    if (contentType.toLowerCase().startsWith("multipart/form-data")) {
      // keep original boundary, stream body
      headers["Content-Type"] = contentType;
      body = req.body as any;
    } else if (contentType.includes("application/json")) {
      headers["Content-Type"] = "application/json";
      // read as text to avoid stream locking issues
      body = (req as any)
        .clone()
        .text()
        .catch(async () =>
          (await (req as any).clone().json()).then(JSON.stringify).catch(() => "{}")
        );
    } else if (contentType) {
      headers["Content-Type"] = contentType;
      body = req.body as any;
    } else {
      // default to JSON if FE sends none
      headers["Content-Type"] = "application/json";
      body = await req.text().catch(() => "{}");
    }

    // resolve Promise<string> (JSON case)
    if (body && typeof body.then === "function") body = await body;

    const init: RequestInit & { duplex?: "half" } = {
      method: "POST",
      headers,
      body,
      cache: "no-store",
    };
    // streaming multipart needs duplex
    if (body && typeof (body as any).getReader === "function") init.duplex = "half";

    const upstream = await fetch(makeApiUrl(`meta/pages/${encodeURIComponent(pageId)}/tags`), init);

    const data = (await parseSafe(upstream)) ?? {};
    return new NextResponse(typeof data === "string" ? data : JSON.stringify(data), {
      status: upstream.status || 200,
      headers: noStore,
    });
  } catch (e: any) {
    return NextResponse.json(
      { status: "error", message: e?.message || "Proxy POST meta tags failed" },
      { status: 502, headers: noStore }
    );
  }
}
